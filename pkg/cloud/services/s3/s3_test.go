...
func Test_Reconcile_bucket(t *testing.T) {
	...
	t.Run("use_cluster_namespace_and_name_for_bucket_name_by_default", func(t *testing.T) {
		svc, s3Mock := testService(t, infrav1.S3Bucket{
			Enabled: true,
		})
		...
		s3Mock.EXPECT().CreateBucket(gomock.Any()).Do(func(input *s3svc.CreateBucketInput) {
			...
			if !strings.Contains(*input.Bucket, testClusterNamespace) {
				t.Fatalf("Default bucket name should include cluster namespace, got: %q", *input.Bucket)
			}

			if !strings.Contains(*input.Bucket, testClusterName) {
				t.Fatalf("Default bucket name should include cluster name, got: %q", *input.Bucket)
			}
	...
	t.Run("creates_bucket_with_policy_allowing_controlplane_and_worker_nodes_to_read_their_secrets", func(t *testing.T) {
		...
		svc, s3Mock := testService(t, infrav1.S3Bucket{
			Enabled:                        true,
			Name:                           bucketName,
			ControlPlaneIAMInstanceProfile: fmt.Sprintf("control-plane%s", infrav1.DefaultNameSuffix),
			NodesIAMInstanceProfiles: []string{
				fmt.Sprintf("nodes%s", infrav1.DefaultNameSuffix),
			},
		})
	...
	t.Run("returns_error_when_bucket_creation_fails", func(t *testing.T) {
		svc, s3Mock := testService(t, infrav1.S3Bucket{
			Enabled: true,
		})
	...
	t.Run("returns_error_when_creating_bucket_policy_fails", func(t *testing.T) {
		svc, s3Mock := testService(t, infrav1.S3Bucket{
			Enabled: true,
		})

		s3Mock.EXPECT().CreateBucket(gomock.Any()).Return(nil, nil).Times(1)
		s3Mock.EXPECT().PutBucketPolicy(gomock.Any()).Return(nil, errors.New("error")).Times(2)
...
func Test_Delete_bucket(t *testing.T) {
	...
	t.Run("deletes_bucket_with_cluster_name_by_default", func(t *testing.T) {
		svc, s3Mock := testService(t, infrav1.S3Bucket{
			Enabled: true,
		})

		s3Mock.EXPECT().DeleteBucket(gomock.Any()).Do(func(input *s3svc.DeleteBucketInput) {
			if input.Bucket == nil {
				t.Fatalf("DeleteBucket request must have Bucket specified")
			}
	...
	t.Run("returns_error_when_bucket_removal_fails_for_any_reason", func(t *testing.T) {
		svc, s3Mock := testService(t, infrav1.S3Bucket{
			Enabled: true,
		})
...
func Test_Create(t *testing.T) {
	...
	t.Run("when_creating_object_for_control-plane_machine", func(t *testing.T) {
		...
		s3Mock.EXPECT().PutObject(gomock.Any()).Do(func(putObjectInput *s3svc.PutObjectInput) {
			t.Run("use_configured_bucket_name_on_cluster_level", func(t *testing.T) {
				if *putObjectInput.Bucket != expectedBucketName {
					t.Fatalf("Expected object to be created in bucket %q, got %q", expectedBucketName, *putObjectInput.Bucket)
				}
			})

			t.Run("use_machine_role_and_machine_name_as_key", func(t *testing.T) {
				if !strings.HasPrefix(*putObjectInput.Key, "control-plane") {
					t.Errorf("Expected key to start with control-plane role, got: %q", *putObjectInput.Key)
				}
		...
		t.Run("returns_s3_url_for_created_object", func(t *testing.T) {
			...
			if !strings.Contains(parsedURL.Host, expectedBucketName) {
				t.Errorf("URL Host should include bucket %q reference, got %q", expectedBucketName, parsedURL.Host)
			}
...
func Test_Delete(t *testing.T) {
	...
	t.Run("when_deleting_object_for_control-plane_machine", func(t *testing.T) {
		expectedBucketName := "foo"
