...
func Test_Reconcile_bucket(t *testing.T) {
	...
	t.Run("use_cluster_name_for_bucket_name_by_default", func(t *testing.T) {
		expectedBucketName := testClusterName
	...
	t.Run("ignores_when_bucket_already_exists_but_it's_owned_by_the_same_account", func(t *testing.T) {
		svc, s3Mock := testService(t, infrav1.S3Bucket{
			Enabled: true,
		})
...
func Test_Delete_bucket(t *testing.T) {
	...
	t.Run("deletes_bucket_with_cluster_name_by_default", func(t *testing.T) {
		...
		input := &s3svc.DeleteBucketInput{
			Bucket: aws.String(testClusterName),
		}
