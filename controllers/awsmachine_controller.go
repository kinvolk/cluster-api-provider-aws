...
type AWSMachineReconciler struct {
	...
	objectStoreServiceFactory    func(cloud.ClusterScoper) services.ObjectStore
...
func (r *AWSMachineReconciler) createInstance(ec2svc services.EC2MachineInterface, machineScope *scope.MachineScope, clusterScope cloud.ClusterScoper, objectStoreSvc services.ObjectStore) (*infrav1.Instance, error) {
	machineScope.Info("Creating EC2 instance")

	userData, userDataErr := r.resolveUserData(machineScope, clusterScope, objectStoreSvc)
	if userDataErr != nil {
...
func (r *AWSMachineReconciler) resolveUserData(machineScope *scope.MachineScope, clusterScope cloud.ClusterScoper, objectStoreSvc services.ObjectStore) ([]byte, error) {
	...
	if machineScope.UseSecretsManager(userDataFormat) {
		return r.cloudInitUserData(machineScope, clusterScope, userData)
	}

	if machineScope.UseIgnition(userDataFormat) {
		return r.ignitionUserData(machineScope, objectStoreSvc, userData)
	}
